...
function normalizeApiUrl(rawUrl: string | undefined): string | null {
  ...
  try {
    ...
    parsed.pathname = normalizedPath === "" ? "/analyze" : normalizedPath;
...
export async function POST(request: Request) {
  ...
  const endpoint = normalizeApiUrl(process.env.NEXT_PUBLIC_API_URL || process.env.ANALYZE_API_URL);
    ...
    return NextResponse.json(
      {
        error:
          ...
          "Set NEXT_PUBLIC_API_URL to your backend endpoint (e.g. https://your-backend.onrender.com/analyze). " +
  ...
  try {
    ...
    return NextResponse.json({ error: "Unable to reach analysis service right now." }, { status: 502 });
  }
}
