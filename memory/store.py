...
class MemoryStore:
    ...
    def save_run(self, result: ResearchRunResult) -> None:
        ...
        if len(metadata_raw) > _METADATA_CHAR_LIMIT:
            # Truncate to valid JSON by storing a summary object instead
            metadata_raw = json.dumps({"truncated": True, "score": getattr(result, 'score', 0), "verdict": str(getattr(result, 'verdict', 'unknown'))}, ensure_ascii=False)
        with self._connect() as connection:
            connection.execute(
                """
