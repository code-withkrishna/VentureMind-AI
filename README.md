...
**Fallback**: If `ANALYZE_API_URL` is not set, the frontend enters **demo mode** — it runs the full UI with illustrative analysis so judges can experience the product without a backend.
...
```bash
# In a new terminal:
ANALYZE_API_URL=http://localhost:8000/analyze npm run dev
# Frontend live at: http://localhost:3000
```

---

## Production deployment (recommended: Vercel + Railway)

### Backend → Railway (free tier)

1. Go to https://railway.app → New Project → Deploy from GitHub
2. Select this repo, set **root directory** to `/` (the Python root)
...
4. Add environment variables:
   ...
   SERPER_API_KEY=...
   ```
5. Deploy — Railway gives you a URL like `https://venturemind-ai.up.railway.app`
...
3. Add environment variable:
   ```
   ANALYZE_API_URL=https://venturemind-ai.up.railway.app/analyze
   ```
4. Deploy — done. Your live URL is `https://venturemind-ai.vercel.app`
...
| `ANALYZE_API_URL` | Next.js only | URL of `api_server.py` POST endpoint |
